<template>
  <div class="content-audit">
    ...
    <el-dialog v-model="clubDetailVisible" title="社团申请详情" width="700px">
      <div v-if="currentClub" class="detail-container">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="申请人">{{ currentClub.leader?.realName }}</el-descriptions-item>
          <el-descriptions-item label="申请时间">{{ currentClub.createdAt }}</el-descriptions-item>
    ...
    <!-- 活动详情弹窗 -->
    <el-dialog v-model="activityDetailVisible" title="活动申请详情" width="700px">
      <div v-if="currentActivity" class="detail-container">
        <el-descriptions :column="2" border>
          <el-descriptions-item label="活动名称">{{ currentActivity.name }}</el-descriptions-item>
          <el-descriptions-item label="所属社团">{{ currentActivity.club?.name }}</el-descriptions-item>
          <el-descriptions-item label="开始时间">{{ currentActivity.date }}</el-descriptions-item>
          <el-descriptions-item label="结束时间">{{ currentActivity.endAt }}</el-descriptions-item>
          <el-descriptions-item label="活动地点">{{ currentActivity.location || '未填写' }}</el-descriptions-item>
          <el-descriptions-item label="发布人">{{ currentActivity.leaderName }}</el-descriptions-item>
...
const fetchAuditData = async () => {
  loading.value = true
  try {
    const [clubs, activities] = await Promise.all([
      getPendingClubs(),
      getPendingActivities()
    ])
    pendingClubs.value = clubs
    pendingActivities.value = activities
  } catch (error: any) {
...
const handleApproveClub = (row: any) => {
  ElMessageBox.confirm(`确定要通过社团 ${row.name} 的创建申请吗？`, '审批提示', { type: 'success' }).then(async () => {
    try {
...
const handleRejectClub = (row: any) => {
  ElMessageBox.prompt(`请输入拒绝社团 ${row.name} 的原因`, '审批提示', { type: 'warning' }).then(async (data: any) => {
    try {
      await auditClub(row.id, { status: Status.REJECTED, reason: data.value })
...
const handleApproveActivity = (row: any) => {
  ElMessageBox.confirm(`确定要通过活动 ${row.name} 的发布申请吗？`, '审批提示', { type: 'success' }).then(async () => {
    try {
...
const handleRejectActivity = (row: any) => {
  ElMessageBox.prompt(`请输入拒绝活动 ${row.name} 的原因`, '审批提示', { type: 'warning' }).then(async (data: any) => {
    try {
      await auditActivity(row.id, { status: ActivityStatus.REJECTED, reason: data.value })
      ElMessage.warning(`已拒绝申请，原因: ${data.value}`)
      fetchAuditData()
    } catch (error: any) {
      ElMessage.error(error.message || '操作失败')
